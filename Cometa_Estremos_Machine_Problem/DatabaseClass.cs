...
namespace Cometa_Estremos_Machine_Problem
{
    public class DatabaseClass
    {
        ...
        {
            request = (HttpWebRequest)WebRequest.Create("http://192.168.1.4/MoneySendingApp/Functions/search_user_record.php?mobile_num=" + mobileNum);
            response = (HttpWebResponse)request.GetResponse();
        ...
        public void RecordTransaction(string mobileNum, string newBalance, string transType)
        {
            ...
            QueryCommand("http://192.168.1.4/MoneySendingApp/Functions/record_transaction.php?user_mobile_num=" + mobileNum
        ...
        public string[] GetUserTransHistory(string mobileNum)
        {
            ...
            try
            {
                request = (HttpWebRequest)WebRequest.Create("http://192.168.1.4/MoneySendingApp/Functions/search_trans_history.php?user_mobile_num=" + mobileNum);
                response = (HttpWebResponse)request.GetResponse();
                res = response.ProtocolVersion.ToString();
                StreamReader reader = new StreamReader(response.GetResponseStream());
                var result = reader.ReadToEnd();
